...
import { 
  MapPin, Clock, Coffee, ShoppingBag, Camera, 
  Plane, Utensils, Hotel, ArrowUp, Calendar, Moon, Upload, 
  Image as ImageIcon, CheckSquare, Sun, CloudSun, AlertTriangle,
  Phone, BookOpen, Car
} from 'lucide-react';
import { 
  ITINERARY, TRIP_DETAILS, CHECKLIST_DATA, FOOD_HIGHLIGHTS, MARKET_HIGHLIGHTS, TRAVEL_TIPS,
  DaySchedule, ItineraryItem, HighlightItem 
  ...
  return (
    <div className="min-h-screen bg-thai-bg text-thai-dark selection:bg-thai-pink selection:text-white font-sans">
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-white/90 backdrop-blur-md shadow-sm py-3' : 'bg-transparent py-6'}`}>
        ...
        {/* Mobile Nav */}
        <div className="lg:hidden flex overflow-x-auto gap-2 px-4 pb-2 mt-2 hide-scrollbar">
           {ITINERARY.map((day) => (
               <button
                 key={day.id}
                 onClick={() => scrollTo(day.id)}
